...
3. Push commit and tag: `git push && git push --tags` (tag must match `v{version}`, e.g. `v0.1.4`).

The [Release workflow](.github/workflows/release.yml) builds all native targets, runs tests, then publishes `@alexstep/doc-extract` and platform packages (`@alexstep/doc-extract-darwin-arm64`, etc.) to npm.
